...
public class PlayerController {
	...
	public List<PlayerGetDTO> getAllUsers(@RequestHeader(value = "Token", defaultValue = "null") String token) {
		...
		for (Player player : players) {
			playerGetDTOS.add(DTOMapper.INSTANCE.convertEntityToPlayerGetDTO(player)); // convert each player to the API
																						// representation
		}
	...
	public ResponseEntity<PlayerGetDTO> createPlayer(@RequestBody PlayerPostDTO playerPostDTO) {
		Player playerInput = DTOMapper.INSTANCE.convertPlayerPostDTOtoEntity(playerPostDTO); // convert API user to
																								// internal
																								// representation
		...
		return ResponseEntity.created(null).headers(header)
				.body(DTOMapper.INSTANCE.convertEntityToPlayerGetDTO(createdPlayer));
	}

	@GetMapping("/users/{userId}")
	public ResponseEntity<PlayerGetDTO> getPlayer(@PathVariable(value = "userId") Long playerId,
			@RequestHeader(value = "token", defaultValue = "null") String token) {
		playerService.checkToken(token);
		Long id = playerService.getPlayerID(token);
		HttpHeaders header = new HttpHeaders();
		header.set("ID", "" + id);
		header.set("Access-Control-Expose-Headers", "ID"); // allow client to access id (of the process that is asking
															// for data, edit button)
	...
	public ResponseEntity<Void> loginCheck(@RequestParam("username") String username,
	...
	@ResponseStatus(HttpStatus.OK)
	@ResponseBody
	public List<PlayerGetDTO> top15PlayersGamesWon(
			@RequestHeader(value = "Token", defaultValue = "null") String token) {
		playerService.checkToken(token);
	...
	public ResponseEntity<Void> changeUser(@PathVariable(value = "userId") Long userId,
			@RequestHeader(value = "token", defaultValue = "null") String token,
